...
export function RevisionPanel({
  ...
  const handleRestore = async (revision: Revision) => {
    if (
      !confirm(
        `Restore to the version from ${formatDateTime(revision.createdAt)}?\n\nCurrent content will be overwritten.`
      )
    )
      return;

    const success = await restoreRevision(documentId, revision._id);
    if (success) {
      toast.success('Revision restored. Reloading document…');
      onRestored();
      onClose();
    } else {
        ...
        {isLoading ? (
          <div className="flex flex-col gap-2">
            {[...Array(5)].map((_, i) => (
              <div
                key={i}
                className="p-3 rounded-lg bg-surface-overlay animate-pulse"
              >
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {revisions.map((revision, index) => (
              <div
                key={revision._id}
                className="group p-3 rounded-xl bg-surface-overlay border border-border hover:border-border-strong transition-all duration-150"
              >
